...
void onRequest(const qlc::HttpRequest& req, qlc::HttpResponse* resp)
{
  ...
  if (req.path() == "/")
  {
    resp->setStatusCode(qlc::HttpResponse::k200Ok);
    resp->setStatusMessage("OK");
    resp->setContentType("text/html");
    resp->addHeader("Server", "Muduo");
    string now = qlc::Timestamp::now().toFormattedString();
  ...
  {
    resp->setStatusCode(qlc::HttpResponse::k404NotFound);
...
int main(int argc,char**argv){
    ...
    qlc::EventLoop loop;
    qlc::InetAddress addr(4000);
    qlc::HttpServer server(&loop, addr, "dummy");
